...
- 配置为有效非零地址时，应用中心显示入口。
- 未配置时隐藏入口；直接访问路由时显示“当前环境未配置新链发射销毁活动”。
...
```text
Header：新链发射销毁活动
  标题旁显示“活动详情”信息按钮，详情默认收起，点击后在弹窗或移动端抽屉中查看

活动目的：部分伙伴希望在原生代币价值更稳定、流动性更好、链的规则更稳定的其他公链上继续参与LOVE20协议，因此发起本次活动。参与者按最终份额获得新链上首次部署的 LOVE20 代币分配份额，最终部署哪条公链由社区讨论并投票决定。
...
- `airdropTokenAddress == 0`：默认隐藏同链空投区域。
