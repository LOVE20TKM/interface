...
```text
Header：新链发射销毁活动
  标题下显示距离活动开始或结束的轮数（有数据时）

活动详情区域：默认展示完整的活动背景，段落末尾紧跟颜色区分的“展开活动详情”链接；展开后显示其余活动详情和开源合约链接，末尾紧跟“收起活动详情”链接

活动背景：LOVE20协议即将启动多链部署，首个公链暂定为原生代币价值稳定、流动性好、链的规则稳定的BSC公链；最终部署哪条公链由参与销毁的社区成员讨论并投票决定。参与者可通过四种方式销毁/锁定LOVE20协议生态资产（含子币）：流动性质押凭证、加速激励质押凭证、治理激励和行动激励，以获得新链上首次部署的 LOVE20 代币分配份额。

参与资产：活动支持TKM链上部署的LOVE20协议的四类资产：SL 流动性质押凭证、ST 加速激励质押凭证、治理激励和行动激励。SL、ST 会被永久锁定；治理激励和行动激励必须先实际领取并铸造，之后按实际铸造数量生成对应的销毁额度，再销毁对应社区代币。
...
本节的激励领取、凭证锁定和激励代币销毁控件在当前开放轮次及未开始预览中展示；未满足 `isRoundOpen(round)` 时控件保持禁用，不发送交易。交易发送前再次读取或校验轮次状态；交易确认后刷新余额、allowance、额度、统计和份额。活动结束后的 `claimAirdrop()` 不受本轮次门控限制，只依据最终份额和空投状态开放。
...
3. 未开始、开放、历史、累计和已结束状态选择正确；Vote `currentRound <= 2` 时不构造负轮次；未开始时展示当前社区代币、SL 和 ST 余额但不读取 allowance，操作控件可见但禁用。
