...
const BURN_INFO = {
  activityPhase: '根据当前验证轮次判断活动处于未开始、进行中、结算中或已结束。只有进行中的当前开放轮次可以执行锁定和销毁。',
  activityOverview: '汇总本次活动的轮次范围、参与地址、额度倍数、个人份额和同链空投状态。活动结束前的个人份额为实时预估值。',
  ...
  totalShare: '以整个活动的可分配份额为 100%，这是你在所有参与社区、所有活跃资产类别中的份额贡献总和。活动结束前是实时预估值，结束后才最终确定。',
  airdrop: '若活动配置了同链空投代币，最终份额确定后可以领取。可领取数量按领取时的空投池余额、你的最终份额和剩余未领取份额计算。',
  communityShare: '以整个活动的可分配份额为 100%，这是当前社区为你贡献的部分。每个活跃类别先等分当前社区份额，再按你在该类别的得分占社区类别总得分的比例分配。',
  accountCommunityShare: '把当前社区内部视为 100%，活跃资产类别先等分社区份额，再按你在各类别的得分占该类别社区总得分的比例计算并相加。活动结束前会随参与情况变化。',
  scoreBonus: '当前社区在所选轮次的销毁得分额外加成。它只影响得分，不改变销毁额度。计算公式：本轮得分 = 销毁或锁定数量 × 链上得分系数 ÷ 10¹⁸；额外加成 =（链上得分系数 - 10¹⁸）÷ 10¹⁸ × 100%。',
  roundSelector: '选择具体轮次时展示从活动开始截止该轮的累计数量和累计得分；全部轮次展示活动累计。只有当前开放轮次可以执行锁定和销毁。',
  categoryRatio: '以当前社区本类别的截止轮次累计总得分为 100%，这是你的截止轮次累计得分占比。这个比例只比较同一社区、同一资产类别内的参与者。',
...
function CategorySection({
  ...
  const fullScore = (value: bigint) =>
    loading || error ? undefined : `${formatExactAmount(value, decimals)} 分`;
...
export default function BurnPage() {
  ...
  const configuredCommunityShare =
    config.totalCommunityWeight > BigInt(0)
      ? (selectedCommunityWeight * WAD) / config.totalCommunityWeight
      : BigInt(0);
  ...
  if (!isBurnEnabled) {
    return (
      <>
        <Header title="BSC公平发射" showBackButton />
    ...
    <>
      ...
      <main className="mx-auto w-full max-w-5xl px-4 pb-24 pt-3 sm:pt-6">
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-greyscale-900">BSC公平发射</h1>
          ...
          <>
            ...
            <section className="border-y border-greyscale-200 py-5">
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                ...
                />
                <StatValue label="参与地址数" value={config.participantsCount.toString()} info={BURN_INFO.participants} />
                <StatValue label="额度倍数" value={`${config.quotaMultiplier.toString()} 倍`} info={BURN_INFO.quotaMultiplier} />
                <StatValue
              ...
              <div className="mt-4 border-t border-dashed border-greyscale-200 pt-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <InfoLabel label="同链空投" info={BURN_INFO.airdrop} className="text-sm font-semibold text-greyscale-900" />
            ...
            <section className="grid gap-4 py-5 sm:grid-cols-2">
              <div className="space-y-2">
                <InfoLabel label="参与社区" info={communitySelectorInfo} className="text-sm font-medium text-greyscale-700" />
                <Select
              ...
              <div className="space-y-2">
                <InfoLabel label="轮次" info={BURN_INFO.roundSelector} className="text-sm font-medium text-greyscale-700" />
                <Select
                  value={selectedRound}
                  onValueChange={(value) => {
                      ...
                      <>
                        ...
                        {govAmount && (
                          <p className="mt-2 flex flex-wrap items-center gap-1 text-xs text-greyscale-500">
                            ...
                            <span>{scoreMultiplierReady ? `${formatAmount(estimatedGovScore, tokenDecimals)} 分` : '等待得分加成读取完成'}。</span>
                          </p>
                        )}
                      </>
                    ) : (
                      ...
                      maxActionAmount > BigInt(0) ? (
                        <div className="space-y-2">
                          ...
                          <p className="flex flex-wrap items-center gap-1 text-xs text-greyscale-500">
                            ...
                            <span>{formatAmount(maxActionAmount, tokenDecimals)} {tokenSymbol}。</span>
                          </p>
                          {actionAmount && (
                            <p className="flex flex-wrap items-center gap-1 text-xs text-greyscale-500">
                              <InfoLabel label="本次操作预计新增得分" info={BURN_INFO.estimatedScore} />
